...
EXPENSE_CATEGORIES = ["Кредиты", "Здоровье", "Страхование", "Подарок", "Сигареты", "Связь", "Еда", "Транспорт", "Жилье", "Развлечения", "Маркетплейсы", "Путешествия", "Другое"]
...
async def subscribe(message: types.Message):
    ...
    if result:
        has_access, subscription_end = result
        if has_access and subscription_end:  # Проверяем и has_access, и subscription_end
            try:
                subscription_end_date = datetime.strptime(subscription_end, '%Y-%m-%d')
                if subscription_end_date > datetime.now():
                    await message.reply(f"У вас уже есть активная подписка до {subscription_end}. Вы можете продлить её после окончания.")
                    return
                else:
                    # Подписка истекла, но мы можем продлить от её окончания
                    description = f"Продление подписки на {SUBSCRIPTION_DURATION_DAYS} дней (с {subscription_end})"
            except ValueError:
                # Если формат даты неверный, считаем, что подписки нет
                description = f"Доступ к боту на {SUBSCRIPTION_DURATION_DAYS} дней"
        else:
            # Подписки нет или доступ отозван
            description = f"Доступ к боту на {SUBSCRIPTION_DURATION_DAYS} дней"
    else:
        # Подписки нет
        description = f"Доступ к боту на {SUBSCRIPTION_DURATION_DAYS} дней"

    # Создаём сообщение с кнопкой "Дай пять для друзей" (для теста)
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Дай пять для друзей! ✋", callback_data="test_payment")]
    ])
    await message.reply(f"Для теста нажмите на кнопку ниже, чтобы получить доступ.\n\n{description}", reply_markup=keyboard)

@dp.callback_query(lambda c: c.data == "test_payment")
async def process_test_payment(callback: types.CallbackQuery):
    ...
    # Удаляем инлайн-кнопку из сообщения
    await callback.message.edit_text(f"Тестовая оплата прошла успешно! Ваш доступ активирован до {subscription_end_str}.", reply_markup=None)
    await callback.answer("✋ Дай пять! Доступ предоставлен!")

    # Отправляем новое сообщение с основным меню
    keyboard = admin_keyboard if user_id in ADMINS else main_keyboard
    await callback.message.reply("Выберите действие:", reply_markup=keyboard)
...
async def revoke_access(callback: types.CallbackQuery):
    ...
    # Обновляем статус пользователя: сбрасываем has_access и subscription_end
    cursor.execute('UPDATE allowed_users SET has_access = 0, subscription_end = NULL WHERE user_id = ?', (target_user_id,))
    conn.commit()
    ...
    # Уведомляем пользователя
    try:
        await bot.send_message(target_user_id, "Администратор отозвал ваш доступ к боту. Для возобновления доступа обратитесь к администратору или купите подписку, нажав на кнопку 'Оплатить подписку' в главном меню.")
    except Exception as e:
        await bot.send_message(user_id, f"Не удалось уведомить пользователя {target_user_id}: {e}")
